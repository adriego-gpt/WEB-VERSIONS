    ...
    <AnimatePresence>
      <Motion.div className="modal-backdrop modal-backdrop-priority" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
        <Motion.div ref={containerRef} className="sheet legal-sheet" role="dialog" aria-modal="true" aria-labelledby="legal-title" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.2 }} onClick={(event) => event.stopPropagation()}>
          <div className="legal-sheet-header"><div className="legal-header-copy"><h2 id="legal-title">Políticas y condiciones</h2><p className="legal-updated">{brandName} · Actualizadas el 14 de septiembre de 2026</p></div><button type="button" className="icon-btn close-btn" onClick={onClose} aria-label="Cerrar políticas"><X size={18} /></button></div>
          ...
          <div className="legal-sheet-content" role="tabpanel" tabIndex={0} id={`legal-tabpanel-${currentTab}`} aria-labelledby={`legal-tab-${currentTab}`}>
            <div className="legal-body-section">
              ...
              {currentTab === "privacy" && <>
                ...
                <Section title="Proveedores y comunicaciones"><p>Los servicios de alojamiento, base de datos e imágenes procesan información para operar la tienda. El personal autorizado recibe avisos de pedidos por Telegram y, si se habilita, el servicio de automatización configurado. El courier recibe los datos necesarios de entrega; la recuperación de contraseña puede utilizar un proveedor de correo. Estos proveedores pueden comunicar estados de pago o entrega.</p><p>WhatsApp, Google Maps y los enlaces de pago tienen sus propias políticas. Las imágenes externas y Google Fonts reciben datos técnicos al cargar. Algunos proveedores pueden procesar datos fuera de Ecuador y deben aplicarse las garantías exigibles. No vendemos tus datos personales.</p></Section>
              ...
              {currentTab === "cookies" && <>
                ...
                <Section title="Servicios externos y medición"><p>La tienda no incorpora herramientas propias de publicidad ni un servicio externo de analítica. Los eventos internos no incluyen nombre, teléfono, dirección ni comprobantes. Si se incorporan servicios opcionales de medición o publicidad, se informarán y ofrecerán las opciones de consentimiento correspondientes antes de activarlos.</p><p>El mapa de la página principal se carga automáticamente al acercarte a su sección, sin pulsar un botón. En el carrito y los pedidos, el mapa integrado se carga solo cuando eliges verlo. Google puede utilizar cookies y tratar datos técnicos conforme a su <a href="https://policies.google.com/privacy?hl=es" target="_blank" rel="noopener noreferrer">política de privacidad</a>. Al abrir WhatsApp o un enlace de pago también intervienen sus proveedores.</p></Section>
